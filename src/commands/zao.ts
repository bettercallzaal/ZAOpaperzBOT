	...
	const menu = new StringSelectMenuBuilder()
		...
		.addOptions(
			entries.slice(0, 25).map((e, i) => ({
				label:
					e.question.length > 100
						? e.question.slice(0, 97) + "..."
						: e.question,
				value: String(i),
			})),
		);
...
export async function execute(interaction: ChatInputCommandInteraction) {
	...
	if (!question) {
		const embed = new EmbedBuilder()
			...
			.setURL(config.faqUrl)
			.setFooter({
				text: "The ZAO Papers - full read at thezao.xyz/what-is-the-zao",
			});
		await interaction.editReply({
			embeds: [embed],
	...
	if (ragMatches.length > 0) {
		...
		const description =
			top.content.length > 4000
				? top.content.slice(0, 3997) + "…"
				: top.content;
		const embed = new EmbedBuilder()
			.setColor(ACCENT_COLOR)
	...
	await interaction.editReply({
		embeds: [answerEmbed(match.entry.question, match.entry.answer)],
	});
}
