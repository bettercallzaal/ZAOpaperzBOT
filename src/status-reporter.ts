	...
	return {
		...
		message:
			question.length > MAX_MESSAGE_LENGTH
				? question.slice(0, MAX_MESSAGE_LENGTH)
				: question,
		ts: new Date().toISOString(),
		meta: { matched, score },
	};
...
function getClient(): SupabaseClient | null {
	...
	if (!client) {
		logger.warn(
			"SUPABASE_URL/SUPABASE_SERVICE_ROLE_KEY not set - status reporting disabled",
		);
	}
	return client;
}
...
export function logCommandEvent(
	...
	void withTimeout(
		supabase.from("bot_events").insert(payload),
		SUPABASE_TIMEOUT_MS,
	).catch((err) => {
