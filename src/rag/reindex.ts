...
import {
	type ExistingSectionHash,
	findChangedSections,
} from "./diff-sections.js";
...
function getClient(): SupabaseClient | null {
	...
	if (!client) {
		logger.warn(
			"RAG_SUPABASE_URL/RAG_SUPABASE_SERVICE_ROLE_KEY not set - papers reindex disabled",
		);
	}
	return client;
}
...
async function reindexPaper(
	supabase: SupabaseClient,
	paper: PaperListEntry,
): Promise<void> {
	const res = await fetch(paper.url);
	if (!res.ok) {
		logger.warn(
			{ paperId: paper.id, status: res.status },
			"Failed to fetch paper page, skipping",
		);
		return;
	}
...
export async function runReindex(): Promise<void> {
	...
	try {
		...
		logger.warn(
			{ err },
			"Failed to fetch papers.json, skipping this reindex run",
		);
		return;
	}
	...
	for (const paper of papers) {
		try {
			...
			logger.warn(
				{ err, paperId: paper.id },
				"Failed to reindex paper, continuing with next",
			);
		}
	}
}
